...
def cmdLineParse():
    ...
    parser = argparse.ArgumentParser(description='Fetch Sentinel-1 orbits')
    parser.add_argument('-i', '--input-dir', dest='input_dir', type=str, required=True,
                        help='Path to the directory with all Sentinel-1 SAFE files')
    parser.add_argument('-o', '--output', dest='outdir', type=str, required=True,
                        help='Path to output orbit directory')
...
def fetch_orbit(safe_file, orbit_dir):
    ...
    if match is not None:
        output = os.path.join(orbit_dir, matchFileName)
...
if __name__ == '__main__':
    ...
    inps = cmdLineParse()
    safe_files = glob.glob(os.path.join(inps.input_dir, "S1*_IW_SLC__*"))
    for safe_file in safe_files:
        print(f"looking for an orbit file for {safe_file}")
        fetch_orbit(safe_file, inps.outdir)
