...
from compass.s1_rdr2geo import (
    file_name_los_east,
    file_name_los_north,
...
from compass.utils.h5_helpers import (
    DATA_PATH,
    METADATA_PATH,
    TIME_STR_FMT,
    QA_PATH,
    add_dataset_and_attrs,
    Meta,
)
from compass.utils.helpers import WORKFLOW_SCRIPTS_DIR

# determine the path to the world land GPKG file
LAND_GPKG_FILE = os.path.join(
    WORKFLOW_SCRIPTS_DIR, "data", "GSHHS_l_L1.shp.no_attrs.epsg3413_dissolved.gpkg"
)
...
def _compute_slc_array_stats(arr: np.ndarray, pwr_phase: str):
    ...
    return [
        float(np_op(post_op_arr))
        for np_op in [np.nanmean, np.nanmin, np.nanmax, np.nanstd]
    ]
...
def _qa_items_to_h5_and_dict(h5_group, qa_dict, qa_items):
    ...
    for qa_item in qa_items:
        ...
        qa_dict[qa_item.name] = value_description_dict(
            qa_item.value, qa_item.description
        )
...
class QualityAssuranceCSLC:
    ...
    def compute_CSLC_raster_stats(self, cslc_h5py_root, bursts):
        ...
        for b in bursts:
            ...
            for pwr_phase in ["power", "phase"]:
                ...
                _qa_items_to_h5_and_dict(stats_group, pol_dict[pwr_phase], qa_items)
    ...
    def compute_static_layer_stats(self, cslc_h5py_root, rdr2geo_params):
        ...
        static_layers = [key for key, val in static_layers_dict.items() if val]

        self.compute_stats_from_float_hdf5_dataset(
            cslc_h5py_root, static_layer_path, "static_layers", static_layers
        )

    def compute_correction_stats(
        self, cslc_h5py_root, apply_tropo_corrections, tropo_delay_type
    ):
        """
        Compute correction stats. Stats written to HDF5 and saved to class dict
        for later JSON output
        ...
        corrections_src_path = (
            f"{METADATA_PATH}/processing_information/timing_corrections"
        )
        ...
        self.compute_stats_from_float_hdf5_dataset(
            cslc_h5py_root, corrections_src_path, "timing_corrections", corrections
        )

    def compute_stats_from_float_hdf5_dataset(
        self, cslc_h5py_root, src_group_path, qa_group_name, qa_item_names
    ):
        """
        ...
        for qa_item_name in qa_item_names:
            ...
            qa_items = []
            vals = [stat_obj.mean, stat_obj.min, stat_obj.max, stat_obj.sample_stddev]
            for val_name, val in zip(self.stat_names, vals):
                desc = f"{val_name} of {qa_item_name}"
                qa_items.append(Meta(val_name, val, desc))

            # save stats to dict and write to HDF5
            _qa_items_to_h5_and_dict(qa_item_stats_group, qa_item_dict, qa_items)
    ...
    def shadow_pixel_classification(self, cslc_h5py_root):
        ...
        # write items to HDF5 and dict
        _qa_items_to_h5_and_dict(pxl_group, self.pixel_percentage_dict, pxl_qa_items)
    ...
    def percent_land_and_valid_pixels(self, cslc_h5py_root, pol):
        ...
        percent_land_pixels, percent_valid_pixels = (
            self.compute_valid_land_and_pixel_percents(cslc_h5py_root, pol)
        )
        pxl_qa_items = [
            Meta(
                "percent_land_pixels",
    ...
    def populate_rfi_dict(self, cslc_h5py_root, bursts):
        ...
        for burst in bursts:
            ...
            rfi_burst_report_list = [
                Meta("swath", rfi_burst_report["swath"], "Swath of the burst"),
                Meta(
                    "azimuth_time",
                    datetime.datetime.strftime(
                        rfi_burst_report["azimuthTime"], TIME_STR_FMT
                    ),
            ...
            if "timeDomainRfiReport" in rfi_burst_report.keys():
                ...
                burst_time_domain_report_item = [
                    ...
                    Meta(
                        ...
                        "Average percentage of affected level-0 samples in the lines"
                        " containing RFI",
                    ),
                    Meta(
            ...
            if "frequencyDomainRfiBurstReport" in rfi_burst_report.keys():
                ...
                burst_freq_domain_report_item = [
                    ...
                    Meta(
                        "percentage_blocks_persistent_rfi",
                        freq_domain_report["percentageBlocksPersistentRfi"],
                        "Percentage of processing blocks affected by persistent RFI. In"
                        " this case the RFI detection is performed on the mean PSD of"
                        " each processing block",
                    ),
                    Meta(
                        "max_percentage_bw_affected_persistent_rfi",
                ...
                rfi_burst_report_freq_domain_group = (
                    rfi_burst_report_group.require_group("frequency_domain_rfi_report")
                )
                _qa_items_to_h5_and_dict(
                    rfi_burst_report_freq_domain_group,
                    self.rfi_dict["rfi_burst_report"]["frequency_domain_rfi_report"],
                ...
                isolated_rfi_report_group = (
                    rfi_burst_report_freq_domain_group.require_group(
                        "isolated_rfi_report"
                    )
                )
                _qa_items_to_h5_and_dict(
    ...
    def compute_valid_land_and_pixel_percents(self, cslc_h5py_root, pol):
        ...
        mask_land = _get_land_mask(
            epsg_cslc, (x0, x_spacing, 0, y0, 0, y_spacing), (height_cslc, width_cslc)
        )
    ...
    def compute_layover_shadow_pixel_percent(self, cslc_h5py_root):
        ...
        mask_shadow_inside_burst = mask_geocoded_burst & (
            layover_shadow_mask_array == 1
        )
        percent_shadow = mask_shadow_inside_burst.sum() / n_unmasked_pxls * 100

        mask_layover_inside_burst = mask_geocoded_burst & (
            layover_shadow_mask_array == 2
        )
        percent_layover = mask_layover_inside_burst.sum() / n_unmasked_pxls * 100

        mask_combined_inside_burst = mask_geocoded_burst & (
            layover_shadow_mask_array == 3
        )
...
def _get_valid_pixel_mask(arr_cslc):
    ...
    labels_along_edges = np.concatenate(
        (labeled_arr[0, :], labeled_arr[-1, :], labeled_arr[:, 0], labeled_arr[:, -1])
    )
...
def _get_land_mask(epsg_cslc: int, geotransform: tuple, shape_mask: tuple):
    ...
    ds_intersection_polygon = drv_intersection_polygon.CreateDataSource(
        str(time.time_ns)
    )
    layer_intersection = ds_intersection_polygon.CreateLayer(
        "layer_intersection", srs_cslc, ogr.wkbPolygon
    )
    ...
    rasterized_land = drv_raster_out.Create(
        str(time.time_ns), shape_mask[1], shape_mask[0], 1, gdal.GDT_Byte
    )
