...
def _fix_layover_shadow_mask(static_layers_dict, h5_root, geo_grid, output_params):
    """
    ...
    if correctly_masked_dataset_name:
        ...
        correctly_masked_dataset_arr = h5_root[
            f"{DATA_PATH}/{correctly_masked_dataset_name}"
        ][()]
...
def run(cfg, burst, fetch_from_scratch=False):
    ...
    static_layers = {
        file_name_x: (
            cfg.rdr2geo_params.compute_longitude,
            "x",
            "Longitude coordinate in degrees",
        ),
        file_name_y: (
            cfg.rdr2geo_params.compute_latitude,
            "y",
            "Latitude coordinate in degrees",
        ),
        file_name_z: (cfg.rdr2geo_params.compute_height, "z", "Height in meters"),
        file_name_local_incidence: (
            cfg.rdr2geo_params.compute_local_incidence_angle,
            "local_incidence_angle",
            "Local incidence angle in degrees",
        ),
        file_name_los_east: (
            cfg.rdr2geo_params.compute_ground_to_sat_east,
            "los_east",
            "East component of LOS unit vector from target to sensor",
        ),
        file_name_los_north: (
            cfg.rdr2geo_params.compute_ground_to_sat_north,
            "los_north",
    ...
    with h5py.File(out_h5, "w") as h5_root:
        ...
        for dataset_name, (
            ...
            topo_ds = init_geocoded_dataset(
                static_layer_data_group,
                dataset_name,
                geo_grid,
                dtype,
                description,
            ...
            output_raster = isce3.io.Raster(
                f"IH5:::ID={topo_ds.id.id}".encode("utf-8"), update=True
            )
            ...
            if dataset_name == "layover_shadow_mask":
                _fix_layover_shadow_mask(
                    static_layers, h5_root, geo_grid, cfg.output_params
                )
...
def geocode_luts(
    ...
    dst_group = geo_burst_h5.require_group(dst_group_path)
    ...
    radargrid_interp = get_decimated_rdr_grd(
        burst.as_isce3_radargrid(), dec_factor_x_rng, dec_factor_y_az
    )
    ...
    for item_name, (
        ...
        geocoded_cal_lut_raster = isce3.io.Raster(
        ...
        param_interp_obj_rg = InterpolatedUnivariateSpline(rg_lut_grid, rg_lut_val, k=1)
        ...
        if az_lut_grid is None or az_lut_val is None:
            ...
            param_interp_obj_az = InterpolatedUnivariateSpline(
                az_lut_grid, az_lut_val, k=1
            )
            azimuth_lut_interp = param_interp_obj_az(azimuth_px_interp_vec)

        lut_arr = np.matmul(
            azimuth_lut_interp[..., np.newaxis], range_lut_interp[np.newaxis, ...]
        )

        lut_path = f"{scratch_path}/{item_name}_radargrid.tif"
        lut_gdal_raster = gdal_geotiff_driver.Create(
            lut_path,
...
def geocode_calibration_luts(
    geo_burst_h5, burst, cfg, dec_factor_x_rng=20, dec_factor_y_az=5
):
    """
    Geocode the radiometric calibration parameters,
    and write them into output HDF5.
...
if __name__ == "__main__":
    ...
    cfg = GeoRunConfig.load_from_yaml(
        parser.args.run_config_path, workflow_name="s1_cslc_geo"
    )
