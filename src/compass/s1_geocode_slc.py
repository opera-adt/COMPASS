...
def run(cfg: GeoRunConfig):
    ...
    for burst_id, bursts in bursts_grouping_generator(cfg.bursts):
        ...
        if cfg.lut_params.enabled:
            rg_lut, az_lut = cumulative_correction_luts(
                burst,
                dem_path=cfg.dem,
        ...
        b_bounds = np.s_[
            burst.first_valid_line : burst.last_valid_line,
        ...
        with h5py.File(output_hdf5, "w") as geo_burst_h5:
            ...
            for burst in bursts:
                ...
                if check_eap.phase_correction:
                    temp_slc_path_corrected = temp_slc_path.replace(
                        "_temp.vrt", "_corrected_temp.rdr"
                    )

                    apply_eap_correction(
                        burst, temp_slc_path, temp_slc_path_corrected, check_eap
                    )
            ...
            geo_datasets.extend([carrier_phase_ds, flatten_phase_ds])
            geo_data_blks.extend(
                [
                    _wrap_phase(carrier_phase_data_blk),
                    _wrap_phase(flatten_phase_data_blk),
                ]
            )
            for cslc_dataset, cslc_data_blk in zip(geo_datasets, geo_data_blks):
        ...
        with h5py.File(output_hdf5, "a") as geo_burst_h5:
            ...
            if cfg.quality_assurance_params.perform_qa:
                ...
                cslc_qa.percent_land_and_valid_pixels(
                    geo_burst_h5, bursts[0].polarization
                )
            ...
            if burst.burst_calibration is not None:
                ...
                s1_geocode_metadata.geocode_calibration_luts(geo_burst_h5, burst, cfg)

            if burst.burst_noise is not None:
                # Geocode the calibration parameters and write them into HDF5
                s1_geocode_metadata.geocode_noise_luts(geo_burst_h5, burst, cfg)
        dt_qa_meta = get_time_delta_str(t_qa_meta)

    dt = get_time_delta_str(t_start)
    info_channel.log(
        f"{module_name} corrections computation time {dt_corrections} (hr:min:sec)"
    )
