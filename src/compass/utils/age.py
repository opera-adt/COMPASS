...
def run(
    ...
    for idx, row in cr_df.iterrows():
        ...
        if cslc_poly.contains(buff_cr_loc):
            ...
            cslc_epsg = get_cslc_epsg(cslc_file, mission_id=mission_id, pol=pol)

            # Correct corner reflector position for solid Earth tides
            # otherwise just transform coordinates to UTM
            if correct_set:
                x, y = correct_cr_tides(
                    cslc_file, cr_lat, cr_lon, mission_id=mission_id, pol=pol
                )
            else:
            ...
            x_start, dx, y_start, dy = get_xy_info(
                cslc_file, mission_id=mission_id, pol=pol
            )
    ...
    arr = get_cslc(cslc_file, mission_id=mission_id, pol=pol)
    ...
    for idx, row in cr_df.iterrows():
        x_peak, y_peak, snr_cr = find_peak(
            arr,
            cslc_file,
            int(row["CR_X_CSLC"]),
            int(row["CR_Y_CSLC"]),
    ...
    if plot_age:
        ...
        ax.scatter(cr_df["ALE_X"], cr_df["ALE_Y"], s=200, alpha=0.8, marker="o")
...
def correct_cr_tides(cslc_file, cr_lat, cr_lon, mission_id="S1", pol="VV"):
    """
    Correct Corner reflector position for Solid Earth tides
    Parameters
    ----------
    ...
    if mission_id == "S1":
        start_path = (
            f"{METADATA_PATH}/processing_information/input_burst_metadata/sensing_start"
        )
        stop_path = (
    ...
    sensing_start = dt.datetime.strptime(start.decode("UTF-8"), TIME_STR_FMT)
    sensing_stop = dt.datetime.strptime(stop.decode("UTF-8"), TIME_STR_FMT)

    # Compute SET in ENU using pySolid
    (_, tide_e, tide_n, _) = pysolid.calc_solid_earth_tides_point(
        cr_lat,
        cr_lon,
        sensing_start,
        sensing_stop,
...
def find_peak(
    arr, cslc_file, x_loc, y_loc, pol="VV", mission_id="S1", ovs_factor=128, margin=32
):
    """
    ...
    x_start, x_spac, y_start, y_spac = get_xy_info(
        cslc_file, mission_id=mission_id, pol=pol
    )
    ...
    if (
        (upperleft_x < 0)
        or (upperleft_y < 0)
        or (lowerright_x > arr.shape[1])
        or (lowerright_y > arr.shape[0])
    ):
    ...
    img_ovs = isce3.cal.point_target_info.oversample(img, ovs_factor)
...
def get_snr_cr(img: np.ndarray, cutoff_percentile: float = 3.0):
    ...
    mask_threshold = np.logical_and(power_arr < thres_low, power_arr > thres_high)
    mask_invalid_pixel = np.logical_and(power_arr <= 0.0, np.isnan(power_arr))
    ma_power_arr = np.ma.masked_array(
        power_arr, mask=np.logical_and(mask_threshold, mask_invalid_pixel)
    )
...
def create_parser():
    ...
    optional.add_argument(
        ...
        default=False,
        help=(
            "If True, corrects CSV corner reflector positions for Solid Earth Tides"
            " (default: False)"
        ),
    )
    optional.add_argument(
    ...
    optional.add_argument(
        ...
        type=int,
        help=(
            "Oversample factor for determining CR location in the "
    ...
    optional.add_argument(
        ...
        type=int,
        help=(
            "Padding margin around CR position detected in the geocoded SLC "
    ...
    optional.add_argument(
        ...
        default=True,
        help=(
            "If True, removes azimuth carrier ramp prior to CR peak location. "
    ...
    optional.add_argument(
        ...
        default=True,
        help=(
            "If True, adds back the flatten phase. This option should be set"
