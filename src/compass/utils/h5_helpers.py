...
def init_geocoded_dataset(
    grid_group, dataset_name, geo_grid, dtype, description, data=None, output_cfg=None
):
    """
    ...
    if data is None:
        cslc_ds = grid_group.require_dataset(
            dataset_name, dtype=dtype, shape=shape, **output_kwargs
        )
    else:
        cslc_ds = grid_group.create_dataset(dataset_name, data=data, **output_kwargs)
    ...
    x_ds = grid_group.require_dataset(
        "x_coordinates", dtype="float64", data=x_vect, shape=x_vect.shape
    )
    y_ds = grid_group.require_dataset(
        "y_coordinates", dtype="float64", data=y_vect, shape=y_vect.shape
    )
    ...
    # UTM zones
        ...
        # Set up grid mapping
        projection_ds.attrs["grid_mapping_name"] = np.string_(
            "universal_transverse_mercator"
        )
        projection_ds.attrs["utm_zone_number"] = geo_grid.epsg % 100

        # Setup units for x and y
        x_ds.attrs["description"] = np.string_(
            "CF compliant dimension associated with the X coordinate"
        )
        ...
        y_ds.attrs["description"] = np.string_(
            "CF compliant dimension associated with the Y coordinate"
        )
...
def save_orbit(orbit, orbit_direction, orbit_type, orbit_group):
    ...
    orbit_items = [
        Meta(
            "reference_epoch",
            ref_epoch,
            "Reference epoch of the state vectors",
            {"format": "YYYY-MM-DD HH:MM:SS.6f"},
        ),
        Meta(
            "time",
            np.linspace(orbit.time.first, orbit.time.last, orbit.time.size),
    ...
    orbit_ds = orbit_group.require_dataset(
        "orbit_type", (), "S10", data=np.string_(orbit_type)
    )
    orbit_ds.attrs["description"] = np.string_(
...
def identity_to_h5group(dst_group, burst, cfg, product_type):
    ...
    id_meta_items = [
        ...
        Meta(
            "product_level",
            "L2",
            "L0A: Unprocessed instrument data; L0B: Reformatted, unprocessed instrument"
            " data; L1: Processed instrument data in radar coordinates system; and L2:"
        ...
        Meta(
            "is_geocoded",
            "True",
            "Boolean indicating if product is in radar geometry or geocoded",
        ),
        Meta(
            "processing_center",
            "Jet Propulsion Laboratory",
            "Name of the processing center that produced the product",
        ),
    ]
...
def metadata_to_h5group(
    ...
    save_orbit(burst.orbit, burst.orbit_direction, orbit_type, orbit_group)
    ...
    if burst.burst_calibration is not None and save_noise_and_cal:
        ...
        cal_items = [
            Meta(
                "azimuth_time",
                cal.azimuth_time.strftime(TIME_STR_FMT),
                "Start time",
                {"format": "YYYY-MM-DD HH:MM:SS.6f"},
            ),
    ...
    processing_group["runconfig"].attrs["description"] = np.string_(
        "Run configuration file used to generate the CSLC-S1 product"
    )
    ...
    input_items = [
        ...
        Meta(
            "orbit_files", orbit_files, "List of input orbit files used for processing"
        ),
        Meta(
            "calibration_files",
            burst.burst_calibration.basename_cads,
            "List of input calibration files used for processing",
        ),
        Meta(
            "noise_files",
            burst.burst_noise.basename_nads,
    ...
    vrt_items = [
        ...
        Meta(
            "burst_index", burst.i_burst, "Burst index relative other bursts in swath"
        ),
        Meta(
            "first_valid_sample",
    ...
    burst_meta_items = [
        ...
        Meta(
            "shape",
            burst.shape,
            "Shape (length, width) of the burst in radar coordinates",
            {"units": "pixels"},
        ),
        Meta(
            "range_bandwidth",
            burst.range_bandwidth,
            "Slant range bandwidth of the signal",
            {"units": "Hertz"},
        ),
        ...
        Meta(
            "center",
            [xy[0] for xy in burst.center.coords.xy],
            "Longitude, latitude center of burst",
            {"units": "degrees"},
        ),
        # window parameters
        Meta(
        ...
        Meta(
            "rank",
            burst.rank,
            "The number of Pulse Repetition Intervals (PRI) between transmitted pulse"
            " and return echo",
        ),
        Meta(
            "prf_raw_data",
    ...
    if save_processing_parameters:
        dry_tropo_corr_enabled = (
            True
            if (cfg.weather_model_file is not None)
            and ("dry" in cfg.tropo_params.delay_type)
            else False
        )
        wet_tropo_corr_enabled = (
            True
            if (cfg.weather_model_file is not None)
            and ("wet" in cfg.tropo_params.delay_type)
            else False
        )
        ...
        par_meta_items = [
            Meta(
                "ellipsoidal_flattening_applied",
                bool(cfg.geocoding_params.flatten),
                "If True, CSLC-S1 phase has been flattened with respect to a zero"
                " height ellipsoid",
            ),
            Meta(
                "elevation_antenna_pattern_correction_applied",
            ...
            Meta(
                "topographic_flattening_applied",
                bool(cfg.geocoding_params.flatten),
                "If True, CSLC-S1 phase has been flattened with respect to topographic"
                " height using a DEM",
            ),
            Meta(
            ...
            Meta(
                "azimuth_fm_rate_applied",
                bool(cfg.lut_params.enabled),
                "If True, azimuth FM-rate mismatch timing correction has been applied",
            ),
            Meta(
                "geometry_doppler_applied",
                bool(cfg.lut_params.enabled),
                "If True, geometry steering doppler timing correction has been applied",
            ),
            Meta(
                "los_solid_earth_tides_applied",
                bool(cfg.lut_params.enabled),
                "If True, solid Earth tides correction has been applied in slant range"
                " direction",
            ),
            Meta(
                "azimuth_solid_earth_tides_applied",
                False,
                "If True, solid Earth tides correction has been applied in azimuth"
                " direction",
            ),
            Meta(
                "static_troposphere_applied",
                bool(cfg.lut_params.enabled),
                "If True, troposphere correction based on a static model has been"
                " applied",
            ),
            Meta(
                "ionosphere_tec_applied",
                tec_corr_enabled,
                "If True, ionosphere correction based on TEC data has been applied",
            ),
            Meta(
                "dry_troposphere_weather_model_applied",
                dry_tropo_corr_enabled,
                "If True, dry troposphere correction based on weather model has been"
                " applied",
            ),
            Meta(
                "wet_troposphere_weather_model_applied",
                wet_tropo_corr_enabled,
                "If True, wet troposphere correction based on weather model has been"
                " applied",
            ),
        ]
...
def algorithm_metadata_to_h5group(parent_group, is_static_layers=False):
    ...
    algorithm_items = [
        ...
        Meta(
            "float_data_geocoding_interpolator",
            "biquintic interpolation",
            "Floating-point data geocoding interpolation method",
        ),
        Meta("ISCE3_version", isce3.__version__, "ISCE3 version used for processing"),
        Meta(
            "s1_reader_version",
            s1reader.__version__,
    ...
    if is_static_layers:
        algorithm_items.extend(
            [
                Meta(
                    "uint_data_geocoding_interpolator",
                    "nearest neighbor interpolation",
                    "Unsigned int geocoding interpolation method",
                ),
                Meta(
                    "topography_algorithm",
                    "isce3.geometry.topo",
    ...
    if not is_static_layers:
        algorithm_items.append(
            Meta(
                "complex_data_geocoding_interpolator",
                "sinc interpolation",
                "Complex data geocoding interpolation method",
            ),
        )
    algorithm_group = parent_group.require_group(
        "metadata/processing_information/algorithms"
    )
    for meta_item in algorithm_items:
        add_dataset_and_attrs(algorithm_group, meta_item)


def corrections_to_h5group(
    parent_group,
    burst,
    cfg,
    rg_lut,
    ...
    ds = gdal.Open(f"{scratch_path}/corrections/corrections", gdal.GA_ReadOnly)
    ...
    correction_items = [
        Meta(
            "slant_range", slant_range, "slant range of LUT data", {"units": "meters"}
        ),
        Meta(
            "slant_range_spacing",
            rg_lut.x_spacing,
            "spacing of slant range of LUT data",
            {"units": "meters"},
        ),
        Meta(
            "zero_doppler_time",
            azimuth,
            "azimuth time of LUT data",
            {"units": "seconds"},
        ),
        Meta(
            "zero_doppler_time_spacing",
            rg_lut.y_spacing,
            "spacing of azimuth time of LUT data",
            {"units": "seconds"},
        ),
        Meta(
            "bistatic_delay",
            ds.GetRasterBand(2).ReadAsArray(),
            f"bistatic delay (azimuth) {desc}",
            {"units": "seconds"},
        ),
        Meta(
            "geometry_steering_doppler",
    ...
    fm_rate_aztime_vec = [
        t.strftime(TIME_STR_FMT) for t in extended_coeffs.fm_rate_aztime_vec
    ]
    dc_aztime_vec = [t.strftime(TIME_STR_FMT) for t in extended_coeffs.dc_aztime_vec]
    extended_coeffs_items = [
        Meta(
            "fm_rate_azimuth_time",
            fm_rate_aztime_vec,
            "Azimuth time for FM rate coefficient data",
            {"format": "YYYY-MM-DD HH:MM:SS.6f"},
        ),
        Meta(
            "fm_rate_slant_range_time",
            extended_coeffs.fm_rate_tau0_vec,
            "Slant range time for FM rate coefficient data",
            {"units": "seconds"},
        ),
        Meta(
            "fm_rate_coefficients",
            extended_coeffs.fm_rate_coeff_arr,
            "FM rate coefficient data",
        ),
        Meta(
            "doppler_centroid_azimuth_time",
            dc_aztime_vec,
            "Azimuth time for doppler centroid coefficient data",
            {"format": "YYYY-MM-DD HH:MM:SS.6f"},
        ),
        Meta(
            "doppler_centroid_slant_range_time",
            extended_coeffs.dc_tau0_vec,
            "Slant range time for doppler centroid coefficient data",
            {"units": "seconds"},
        ),
        Meta(
            "doppler_centroid_coefficients",
            extended_coeffs.dc_coeff_arr,
