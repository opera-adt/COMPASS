...
def geocode_slc_params():
    ...
    with open(gslc_template_path, "r") as f_template, open(
        test_params.gslc_cfg_path, "w"
    ) as f_cfg:
        cfg = (
            f_template.read()
            .replace("@TEST_PATH@", str(test_path))
...
def ionex_params(download_data=True):
    ...
    test_params.tec_file = iono.get_ionex_filename(
        test_params.date_str, tec_dir=test_params.tec_dir, sol_code=test_params.sol_code
    )
    ...
    if download_data:
        if not os.path.isfile(test_params.tec_file):
            print(
                f"Download IONEX file at {test_params.date_str} from "
                f"{test_params.sol_code} to {test_params.tec_dir}"
            )
            test_params.tec_file = iono.download_ionex(
                test_params.date_str, test_params.tec_dir, sol_code=test_params.sol_code
            )
